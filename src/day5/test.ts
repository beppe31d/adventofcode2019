...
describe('Part 2', () => {
    it('Calculate simple test', () => {
        ...
        assert.equal(opCode.calculate(prepare(states), [1]).output, 1);
        assert.equal(opCode.calculate(prepare(states), [0]).output, 0);
    });
    ...
    it('Calculate larger test', () => {
        ...
        assert.equal(opCode.calculate(prepare(states), [7]).output, 999);
        assert.equal(opCode.calculate(prepare(states), [8]).output, 1000);
        assert.equal(opCode.calculate(prepare(states), [9]).output, 1001);
    });
});